      ...
      <div className={`w-full ${isMobile ? 'px-2 py-1' : 'px-4 py-2'}`}>
        ...
        <div className={`${isMobile ? 'mt-4 pt-2' : 'mt-6 pt-4'} border-t border-green-900`}>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {['about', 'experience', 'skills', 'contact', 'awards', 'portfolio'].map(cmd => (
              <button
                key={cmd}
                onClick={() => {
                  setInput(cmd === 'portfolio' ? '/portfolio' : cmd);
                  executeCommand(cmd === 'portfolio' ? '/portfolio' : cmd);
                  setInput('');
                }}
                disabled={isProcessing}
